...
public class XlsContactView extends AbstractXlsView {
	...
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		Sheet sheet = workbook.createSheet("Spring MVC Non Template Views - AbstractXlsView");
		// create header row
		...
		contacts.forEach(contact -> {
			...
			row.createCell(1).setCellValue(contact.getContactSSN().toString());
			...
			row.createCell(4).setCellValue(formatter.format(birthDate));
