...
public class PageSourceController {
	...
	public static final String HTML_VIEWS_DIR = "WEB-INF/views/html";
	...
	@ResponseBody
	public String showSource(@PathVariable(name = "resource", required = true) String resource) throws Exception {
		logger.debug("attempting to read resource: '{}'...", resource);
		String path = getWebPath(resource);
		StringBuilder sb = new StringBuilder();
		try (InputStream in = servletContext.getResourceAsStream(path)) {
			if (in == null) {
				String error = String.format("Cannot display page source: page '%s' was not found!", path);
				logger.error(error);
	 ...
	 * @return
	 */
	private String getWebPath(String resource) {
		String extension = FilenameUtils.getExtension(resource);
		String dir = extension.equalsIgnoreCase("html") ? HTML_VIEWS_DIR : JSP_VIEWS_DIR;
